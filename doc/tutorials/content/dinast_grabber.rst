...
* `void (const std::shared_ptr<const pcl::PointCloud<pcl::PointXYZI> >&)`
...
We create a *std::bind* object with the address of the callback *cloud_cb_*, we pass a reference to our DinastProcessor and the argument place holder *std::placeholders::_1*.
