...
class GrabCutHelper : public pcl::GrabCut<pcl::PointXYZRGB>
{
  ...
  public:
  typedef std::shared_ptr<GrabCutHelper > Ptr;
  typedef std::shared_ptr<const GrabCutHelper > ConstPtr;
...
{
//  boost::lock_guard<std::mutex> lock (refine_mutex);
...
{
  //  boost::lock_guard<std::mutex> lock (refine_once_mutex);
...
{
//  boost::lock_guard<std::mutex> lock (fit_gmms_mutex);
